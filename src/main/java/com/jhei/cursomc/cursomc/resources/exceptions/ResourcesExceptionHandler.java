package com.jhei.cursomc.cursomc.resources.exceptions;
...
import com.jhei.cursomc.cursomc.services.exceptions.FileException;
import com.jhei.cursomc.cursomc.services.exceptions.ObjectNotFoundException;

/**
 * @author Jheisson Santos
 * Classe reponsável pelo mapeamento e tratamento de excessões para o tipo ObjectNotFoundException 
 * Trazendo uma mensagem personalizada e faz de forma automática, sendo necessário ter A classe de excessão
 *  e o @ExceptionHandler(ObjectNotFoundException.class)
 */
@ControllerAdvice
public class ResourcesExceptionHandler {
	...
	public ResponseEntity<StandardError> objectNotFound(MethodArgumentNotValidException e, HttpServletRequest request) {
		...
		for(FieldError x: e.getBindingResult().getFieldErrors()) {
			 err.addFieldMessage(x.getField(), x.getDefaultMessage());
		}
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(err);
	}
